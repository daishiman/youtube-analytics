...
thumbnail 通と Cron 役割①(30日超削除)の運用手順、1,100本超テナントの1,000本上限の運用上の扱い、scripts/seed-local.sql を使ったローカル確認手順、migrations/0016_dashboard_media_assets.sql の本番適用手順を runbook にまとめる。P13(本番D1マイグレーションとdeploy)は利用者の明示指示がない限り実行されない可能性がある旨を runbook に明記する。
