...
artifact_subtypes: []
beads_linkage: null
classification_candidates: [{"artifact_kind": "task", "candidate_path": "tasks/feat-skill-analysis-reports/SYS-SAR-P06.md", "confidence": 0.95}]
...
depends_on: ["SYS-SAR-P05"]
domain: "quality"
evaluation_status: "pass"
execution_contexts: []
feature_package_id: "feature-package/feat-skill-analysis-reports"
file_path: "tasks/feat-skill-analysis-reports/SYS-SAR-P06.md"
github_project_linkages: []
github_publication: {"labels": [], "milestone": null, "mode": "local_only", "project_aliases": []}
goal: null
graph_node_id: "SYS-SAR-P06"
...
resource_scope: ["tests/skill-analysis/", "scripts/skill-analysis/", "src/"]
scope_in: []
scope_out: []
source_lineage: {"imported_at": "2026-09-25T00:09:44Z", "origin_kind": "system-dev-planner", "source_digest": "819fd838968d7f4a28f27b5ca133ed1a22d992549cd029677062b253c4372906", "source_path": ".dev-graph/published/feature-package-feat-skill-analysis-reports/task-specs/phase-06-test-run.md", "source_plugin": "system-dev-planner", "source_version": "0.1.0"}
start_date: null
status: "active"
tags: ["p06", "quality", "skill-analysis-reports"]
target_date: null
template_id: "task"
...
- feature_package_id: feature-package/feat-skill-analysis-reports
- parent_feature: feat-skill-analysis-reports
- owners: daishiman / tags: p06, quality, skill-analysis-reports / related_nodes: feat-skill-analysis-reports
- workstream_kind: quality
- build_target_kind: application-code
- depends_on: SYS-SAR-P05
- classification: confidence 0.95 / P06 phase slot への 1対1 写像 / tasks/feat-skill-analysis-reports/SYS-SAR-P06.md
...
P04で用意したテスト・比較スクリプトを実行し、失敗を0件まで解消したうえで、acceptance 12項目それぞれの検証方法が実データ/fixtureで成立することを確認する。

## 背景

実装直後は環境差やタイミング依存の不具合が残りやすいため、P04のテストと比較スクリプトを実行し、実データ/fixtureで不変条件を検証して修正を反映する。
...
- Quality: 主。P04テスト・比較スクリプトの実行と不具合修正
...
- tests/skill-analysis/ 実行結果
- scripts/skill-analysis/ 実行結果
- Consumed artifacts: P05 の実装成果物
- Write scope: tests/skill-analysis/, scripts/skill-analysis/, src/
...
- 検証: node scripts/skill-analysis/verify-analysis-reproducibility.mjs tests/fixtures/skill-analysis-sample --compare-with （直前にPOSTした結果JSON）
- 検証: node .claude/skills/report-design-system/scripts/check-report.mjs tests/fixtures/skill-analysis-sample && node scripts/skill-analysis/check-no-causal-language.mjs tests/fixtures/skill-analysis-sample
...
- 検証: node scripts/skill-analysis/run-yt-analyze-fixture.mjs tests/fixtures/skill-analysis-sample（request_id無し起動、スタブサーバ経由）
...
- 検証: スタブサーバが403/404を返すケースでrunnerが非0終了することを実行して確認する
- 受入: 失敗0件・スキップ0件

以下はP04で設計した検証方法をこのtaskで実行した結果表(test-design.mdの対応表と同一の12行、実行結果列はP07のacceptance.mdで合否を記録する)。
