...
最終更新: 2026-09-25。判定環境はローカル Workers runtime（`pnpm run preview --port 8793` と E2E の port 8794）。8787 は別案件が使用中。commit、push、deploy はしていないので、公開環境の列は未実施。下の既存証跡件数は当時の記録で、今回の全体再検証は `eval-log/elegant-review-20260925/review.md` に記す。
...
| AC4 | 字幕の ON/OFF とスコープの追加・revoke、テナントごと1日4本、検証前は「準備中」 | ローカル実装・テスト合格（日次 Queue、R2保存、PT基準4試行の永続上限）。公開フラグは実チャンネル検証まで未設定 | 未実施 |
...
AC4 は設定画面の準備中表示・ON拒否・既ONのOFFと revoke、機能フラグ下の追加同意を含む。字幕の取得・保存とテナント別1日4本・字幕用1,000 unitsの永続的な上限は日次収集側でローカル実装・テスト済み。実チャンネルと公開環境の受入は未実施。
...
3. **AC4**: 公開フラグが未設定の間は、運営テナントでもトグルが「準備中」で ON にできないことを確かめる。既に ON の場合は OFF にできる。
