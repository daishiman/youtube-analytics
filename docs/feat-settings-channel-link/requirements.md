...
| AC4 | 字幕 ON で force-ssl を追加し、1日5本まで。OFF で revoke して読み取り専用へ戻る。検証前は運営者以外に「準備中」 | auth, security（qa-076, qa-085） | `PUT /api/youtube/captions-auto` | 単体 `captions.test.ts`（10件）、E2E「字幕: 準備中で押せない」 |
