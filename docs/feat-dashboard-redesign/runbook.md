...
2. D1 にテーブルを足す: `pnpm db:migrate:remote`（未適用の 0008〜0021 を適用。0008 は表を作るだけで、既存のデータは変えない）。
...
戻すとき: 画面とコードは前の版を deploy し直す。0008 の表は追加だけで既存の表を変えないので、残したままでよい。
...
```bash
pnpm install
pnpm db:migrate:local            # 0021 までを適用
...
|---|---|---|---|
| owner@example.com | オーナー | 閲覧者 | 全区画と「CSVをアップロード」。レポート詳細・アクション編集は準備中 |
| editor@example.com | 編集者 | — | オーナーと同じ（書込ボタンあり） |
| viewer@example.com | 閲覧者 | — | 数値は同じ。「CSVをアップロード」は出ない。レポート詳細・アクション編集は準備中 |
| other-owner@example.com | — | オーナー | 未連携の空状態（「設定を開く」） |
| partial@example.com | — | — | 自分がオーナーのチャンネルP（YouTube の許可が一部だけ）。ログイン画面の機能の確認用で、ダッシュボードは空状態 |

seed が入れるデータ（テストチャンネルA）: 動画12本（長尺9、Shorts 3。公開は2〜110日前）、直近90日の日次指標、CSV 由来の視聴率、完了した AI 分析レポート1件（発見3つ）、実施中・効果測定中の改善アクション、週次ファネルと目標。日付は `date('now', …)` の相対指定なので、いつ流しても直近のデータになる。サムネイル画像は入れていないので、画面は頭文字の代替表示になる。
...
8. **右列**: 最新AI分析の題名・版・結論・発見と、改善アクションの基準値 → 最新値・期間を確認する。レポート詳細とアクション編集・状態更新は「準備中」と表示され、操作リンクは出ない。
