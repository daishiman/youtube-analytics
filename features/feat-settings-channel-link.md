...
- 字幕自動取得トグル PUT /api/youtube/captions-auto(既定OFF・ON は incremental authorization で youtube.force-ssl を追加、OFF は revoke→読み取り専用で再連携)。収集本体の完成までは全テナントで準備中・ON不可とし、既にONの設定はOFFにできる。完成後の表示上限は1日4本・字幕用1,000 units（captions.list 50 + captions.download 200 units/本）
- 収集本体が完成し公開ゲートを開いた後、force-ssl の検証申請が通るまでは運営者テナントのオーナーだけにONを許可する機能フラグ
...
- YouTube連携の OAuth クライアントをテナントごとに必須で持ち込む(qa-087): GET/PUT/DELETE /api/youtube/google-client と tenant_google_clients(シークレットは TOKEN_ENC_KEY で暗号化・画面/APIに返さない)。未登録のあいだは連携ボタンを押せず、クライアントIDの変更・削除は既存連携を『要再連携』にする
- 利用者向けの表記を『ワークスペース』にし、設定画面に Google Cloud Console の準備手順(8手順+よくあるエラーの対処)を表示する(qa-088)

### 含まない

- 日次収集ジョブ本体と refresh token の暗号化保存・更新(feat-youtube-daily-collection)
- 字幕の取得・保存とテナント別1日4本・字幕用1,000 unitsの永続予算管理(feat-youtube-daily-collection の後続実装)
...
- 実チャンネル検証前で公開フラグが未設定の間は全テナントに『準備中』を示してONを拒否し、既ONのOFFは revoke と読み取り専用の再連携へ進める。ON時の追加同意、字幕の取得・保存、1日4本の永続上限はローカル実装・テスト済みとし、実チャンネルでの受入を後続とする
...
- 根拠章: system-spec/ui-ux.md, system-spec/frontend.md, system-spec/backend.md, system-spec/auth.md, system-spec/security.md, system-spec/database.md, system-spec/maintenance-ops.md(qa-074〜qa-086)
