...
- テナント: 初回ログインで作られるデータ分離の単位。1つの D1 に tenant_id で行分離する。利用者に見せる文言(画面・APIエラー・規約)では『ワークスペース』と表記し、コード・DB・開発者向け文書は tenant/テナントのまま(qa-088)。
