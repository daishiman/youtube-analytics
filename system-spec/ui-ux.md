...
[利用者の決定 qa-102 と実装の観測 qa-103 連動しない区画の明示・登録者の増減の注記・下部タブの余白(2026-09-26)] 最新AI分析と実施中の改善アクションは、対象セレクタの説明文に加えて各カードの説明にも「期間・動画の選択には連動しません。」を表示する(web/pages/dashboard/SidePanels.tsx)。週次ファネルの「登録者の増減(参考)」は、Analytics の太平洋時間の日付をそのままJSTの週に入れた参考値で、週の境目が16〜17時間ずれることを区画内に注記する(web/pages/dashboard/DetailsSection.tsx)。集計は変換せず現行のままとする(qa-102)。幅900px未満の下部タブはラベルが折り返すと高くなるため、AppShell が ResizeObserver で実高さを測ってルート要素の --mobile-nav-height に置き、本文の下余白とトーストの位置に使う(トーストは Shell の外に描画されるためルート要素に置く)。390px幅ではタブが約93pxになり、固定5remの下余白ではフッター末尾の約13pxを隠していたことを E2E が検出し、この変更で解消した。e2e/dashboard.spec.ts は、各カードと週次ファネルの注記、ファネルの取得失敗が区画の中だけに表示されデータ品質とKPIは表示を続けること、900px未満で最下部までスクロールしても下部タブがフッターを覆わないこと(mobile・tablet)を検証する。
