...
|---|---|---|---|---|---|
| presentation | Apple Human Interface Guidelines | 画面設計・操作フロー・情報階層・アクセシビリティの上流原則 | https://developer.apple.com/design/human-interface-guidelines | 2026-07-12 | [qa-024] デザインはFINAL-UI参照の濃紺サイドバー+白カード+ティール主ボタンに統一し、6画面で同一レイアウト骨格を共有(qa-030)。メニュー項目はログイン後の5つ(ダッシュボード・動画・AI分析・改善アクション・設定)で固定し(ログイン画面を含めて6画面)、情報は『最初に見える要点』と『詳しく見る』の2段に分けて一度に出す量を抑える。スマホ・タブレット幅でも全操作をでき(qa-036)、表は縦積みカードに変換する。 |
| application-architecture | Robert C. Martin — Clean Architecture | レイヤ境界・依存方向 (内向き)・ユースケース中心設計 | Clean Architecture (2017), the Dependency Rule | 2026-07-12 | [qa-061/qa-032/qa-033] React + Vite + React Router のSPAをWorkers静的アセットとして配信する。画面はpages/*(6つ: login・dashboard・videos・ai-analysis・actions・settings)+静的ページ2枚(privacy・terms)と、api client層の2層に分け、画面からfetchを直接呼ばずapi client経由にする。状態はURLクエリ(期間・選択ID)とサーバ応答だけで持ち、グローバルストアを置かない。グラフはEChartsを唯一の描画ライブラリとし、サーバが返すグラフ仕様JSONを汎用コンポーネント1つで描画する。qa-061はqa-024の素のTypeScript選択とqa-031の4種限定を置換する。(層の分け方は qa-037 で承認) |
...
グラフの種類が増える前提で、素のTypeScriptを維持するか、EChartsとグラフ仕様JSONを導入するか、React + Vite + React Routerと組み合わせるか。Next.jsまで必要か。

**答**

React + Vite + React Router のSPAを採用し、Workers静的アセット配信と `run_worker_first=/api/*` は維持する。Next.jsは不採用。Apache EChartsを唯一の描画ライブラリとし、サーバが返すグラフ仕様JSONを汎用コンポーネント1つで描画する。qa-031の4種限定は置換する一方、M1〜M10をブラウザで再計算しない方針と「要点だけ先に表示」の情報構成は維持する。

- (根拠の性質: ユーザーの直接選択 / 出所: spec-state.json qa-061 / 回答時刻: 2026-09-22T00:49:08Z)
