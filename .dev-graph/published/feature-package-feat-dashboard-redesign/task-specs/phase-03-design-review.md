...
P02 の設計を、テナント境界(qa-082)・閲覧者の読み取り専用・CSP不変・Cache-Control: private, no-store・Cloudflare Workers Free の subrequest 50・D1 bound parameters 100 の観点でレビューし、他テナント/存在しない video_ids の黙示的除外、1,100本超テナントの1,000本上限、サムネイル未保存時の代替表示の抜けを洗い出す。
