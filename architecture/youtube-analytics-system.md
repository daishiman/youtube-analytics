...
正本: system-spec/ (system-spec-harness 0.1.14・評価 eval-log/completeness-report-20260921-r6.json)。本ノードは要約と参照であり、詳細・根拠(qa_ref)は各章を正とする。qa-060の週次売上ファネルとpsc-001の分析履歴は手動追補済みで、生成lineageの正式再同期は`eval-log/dev-graph-resync-required-20260922.json`をgateとする。
...
- 秘密: wrangler secret のみ(TOKEN_ENC_KEY, GOOGLE_CLIENT_SECRET)。GitHub Secrets は CLOUDFLARE_API_TOKEN(最小権限)と CLOUDFLARE_ACCOUNT_ID。
...
- 無料枠超過 → 無料枠メーターで80%警告、MAX_TENANTS=100。
...
ルートは6画面(ログイン/ダッシュボード/動画/AI分析/改善アクション/設定)+静的ページ2枚(プライバシーポリシー・利用規約)。サイドバー上部にテナント切替。幅900px未満では下部タブ5項目。
...
集約は「レポート版」(reports+findings+psych_findings+comment_emotions)と「改善アクション」(actions)。collector(collectTenantDaily)、ingest(Studio CSV・週次事業CSV・字幕・画像)、tenant 管理、metrics/ を分ける。

## API and service contracts

画面用REST、テナントAPI、/api/skill/*(X-Skill-Api-Version ヘッダ)。`GET /api/skill/export`は週次ファネルと同一tenant+channelの直近5版の履歴射影を返し、`POST /api/skill/reports`は`history_versions_used`を持つ。詳細は specs/youtube-analytics-system.md の API契約。
...
wrangler secret(TOKEN_ENC_KEY, GOOGLE_CLIENT_SECRET)。CLOUDFLARE_API_TOKEN は Workers Scripts/D1/R2 編集のみ。Google Cloud で YouTube Analytics/Reporting/Data API を有効化。
...
`wrangler deploy --dry-run`、28日連続の収集完了監視(O1)、無料枠メーター80%警告。
...
全業務テーブルの主キー・索引の先頭に tenant_id。daily_metrics は PK (tenant_id, channel_id, date, content_type)。`business_funnel_weekly`はPK(tenant_id, channel_id, week_start)、`funnel_targets`はPK(tenant_id, channel_id, metric_id, effective_from)。出典ごとに表を分ける。画像は R2 の tenants/{tenant_id}/ 配下で、キーを media_assets に保存。
...
Google OAuth + PKCE、セッションCookie、個人トークン(SHA-256 保存)。役割の権限表(owner/editor/viewer)を usecase 入口で検査。最後の owner は外せない。
...
Workers Logs、収集失敗の collection_status、削除の再試行。連携解除時は Google revoke を呼びトークンを削除。
